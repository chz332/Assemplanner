...
class Neo4JStorage(BaseGraphStorage):
    ...
    async def _robust_fallback(
        ...
        visited_nodes = set()
        visited_edges = set()
        visited_edge_pairs = set()  # Track processed edge pairs (sorted source_id, target_id)
                ...
                for record in records:
                    ...
                    if edge_id not in visited_edges:
                        ...
                        if target_id:  # Only process if target node has entity_id
                            ...
                            # Sort source_id and target_id to treat (A,B) and (B,A) as the same edge
                            sorted_pair = tuple(sorted([current_node.id, target_id]))

                            # Check if the same edge already exists (considering undirected nature)
                            if sorted_pair not in visited_edge_pairs:
                                # Only add edge if target node is already in results or will be added
